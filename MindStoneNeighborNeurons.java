...
public class MindStoneNeighborNeurons {
    ...
    public static void main (String [] args) {
        ...
        int x = StdIn.readInt();
        java.util.ArrayList<String> one = new ArrayList<>();
        java.util.ArrayList<String> two = new ArrayList<>();
        for (int i =0; i< x ;i++){
            one.add(StdIn.readString());
            two.add(StdIn.readString());
        }
        String mind = "";
        for(String s: two){
            if (!one.contains(s)){
        ...
        for(int i=0; i< one.size();i++){
            if (two.get(i).equals(mind)){
                StdOut.println(one.get(i));
            }
        }
